...
//Constructors
Environment::Environment(sf::RenderWindow *window){ screen = window; }
Environment::~Environment(){ delete screen; }
//Primer
void Environment::start(){ mainLoop(); }
//Main Function Loop
void Environment::mainLoop()
{
	NNetwork_field network(screen);
	network.init();
	//set_learn_weight(network);
	while(screen->isOpen())
	{
...
void Environment::set_learn_weight(NNetwork_field &network)
{
	double input;
	cout << "Please enter new learn weight: ";
	cin >> input;
	network.set_learn_weight(input);
}
//Clear Rendering
void Environment::init(){ screen->clear(); }
